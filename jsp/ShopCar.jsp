...
<%
	...
	try {
		...
		String sql = String.format("select * from shopcar where username='%s'", uname);
		...
		while (rs.next()) {
			table += String.format(
				"<div style='height:7rem; width:45rem; border:1px solid black; margin-bottom:0.5rem;'><a href='GoodsDetail.jsp?isbn=%d'><div style='float:left;border-right:1px solid green;height:7rem;width:7rem'><img src='%s'></img></div></a><div style='float:rigtht; height:7rem; overflow: hidden;'><div style='height:1.3rem;border-bottom:1px solid blue;overflow: hidden;' id='bname'>加入购物车时间： %s</div><div style='height:1.3rem;border-bottom:1px solid blue;overflow: hidden;' id='bname'>书名： %s</div><div style='height:1.3rem;border-bottom:1px solid green;overflow: hidden;' id='bauthor'>作者： %s</div><div style='height:1.3rem;border-bottom:1px solid yellow' id='price'>价格： %.2f</div><div style='height:1.3rem;float:left' id='cnt'>数量： %d</div><div style='float:right'><button onclick='deleteGoods(%d)'>移除</a></button>    <button><a href='GoToPay.jsp?isbn=%d&cnt=%d'>下单</a></button></div></div></div>", rs.getInt("isbn"), rs.getString("picture"), rs.getString("date_buy"),rs.getString("bname"), rs.getString("author"), rs.getDouble("price"),rs.getInt("count"),rs.getInt("isbn"),rs.getInt("isbn"), rs.getInt("count")
				);
		}
		rs.close();
		stmt.close();
