...
<%!Integer kind; String uname = ""; Integer login_kind = 0; String querytext = ""; String table = ""; Integer tt = 0; String types = ""; %>
<%
	...
	String temp = request.getParameter("kind");
...
<head>
	...
	<%
		if (kind == 1) out.print("<title>少儿读物</title>");
		else if (kind == 2) out.print("<title>青春文学</title>");
		else if (kind == 3) out.print("<title>科技</title>");
		else if (kind == 4) out.print("<title>历史</title>");
		else if (kind == 5) out.print("<title>管理</title>");
		else if (kind == 6) out.print("<title>成功励志</title>");
	%>
	...
	<style type="text/css">
		* { margin:0; padding:0; line-height:1.2em; font-family:楷体; }
        ...
        #header_outer { height: 140px; background: url("../images/header_bg.png") repeat-x; border: 1px solid red; }
...
<body>
        ...
        <div id="header" class="wrapper">
            <%
                if (uname.isEmpty()) 
                    out.print("<p id='name'>请您登录</p> ");
                else 
                    out.print("<p id='name'>"+uname+"，欢迎回来！</p> ");
            %>
            <div id="login">
                <%
                    if (login_kind == 1) 
    ...
    <div id = "main_outer">
        <div class = "wrapper" id = "main">
        	<div style="display: none;" id="kindofbook"><%=kind%></div> 
            <%
				if (kind == 1) out.print("<div>少儿读物</div>");
				else if (kind == 2) out.print("<div>青春文学</div>");
				else if (kind == 3) out.print("<div>科技</div>");
				else if (kind == 4) out.print("<div>历史</div>");
				else if (kind == 5) out.print("<div>管理</div>");
				else if (kind == 6) out.print("<div>成功励志</div>");
			%>
